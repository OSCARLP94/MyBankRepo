...
export default function FundsTransfer() {
  ...
  const [destinyProduct, setDestinyProduct] = useState("");
  ...
  const handleCallFundsTransfer = async () => {
    setCallingTransac(true);
    var resp = await ProductService.CallFundsTransferTransaction(
      dispatch,
      ConstValues.DefaultUserTest,
    ...
    {callingTransac ===false ? (
      ...
      >
        ...
        <TextField
          ...
          value={destinyProduct}
          onChange={(e) => setDestinyProduct(e.target.value)}
        />
        <TextField
          sx={{ margin: 1 }}
          required
        ...
        <Button
          variant="outlined"
          disabled={successTransac}
          onClick={async () => handleCallFundsTransfer()}
        >
          Transferir
        </Button>
      </Box>
